...
#comparison = ['r_bt_s2_b128_sb1', 'r_bt_s8_b128_sb4', 'r_bt_s32_b128_sb16', 'r_bt_s128_b128_sb64', 'r_bt_s256_b128_sb128']
...
for c_i, save_dir in enumerate(comparison):
	...
	if cat == 'class':
		if mem == 'LTM':
			plt.plot(smooth(ltm_class_per_batch), label=method[c_i])
		elif mem == 'STM':
			plt.plot(smooth(stm_class_per_batch), label=method[c_i])
		else:
			plt.plot(smooth(total_class_per_batch), label=method[c_i])
	elif cat == 'clip':
		if mem == 'LTM':
			plt.plot(smooth(ltm_clip_per_batch), label=method[c_i])
		elif mem == 'STM':
			plt.plot(smooth(stm_clip_per_batch), label=method[c_i])
		else:
			plt.plot(smooth(total_clip_per_batch), label=method[c_i])
